...
async function main() {
  ...
  const [alice] = await db.insert(usersTable).values({ username: 'alice' }).returning();
  const [bob] = await db.insert(usersTable).values({ username: 'bob' }).returning();
  const [carol] = await db.insert(usersTable).values({ username: 'carol' }).returning();
